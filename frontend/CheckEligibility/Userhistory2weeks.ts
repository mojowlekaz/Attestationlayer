...
async function checkUsage() {
  try {
    ...
    if (activationTimestamp !== null) {
      ...
      const daysInSeconds = 14 * 24 * 3600

      if (timeElapsedSeconds > daysInSeconds) {
        console.log('User has been using Ethereum for over 4 months.')
      } else {
        console.log('User has not been using Ethereum for over 4 months.')
      }
    } else {
